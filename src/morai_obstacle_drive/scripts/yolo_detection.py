...
class YoloNode:
    def __init__(self):
        ...
        # Force GPU inference. Ultralytics was falling back to CPU (nvidia-smi
        # showed no YOLO process on the GPU) -> ~10x slower -> frame rate collapse.
        self.device = 0 if torch.cuda.is_available() else 'cpu'
        self.model.to(self.device)
        if not torch.cuda.is_available():
            rospy.logwarn("YOLO: CUDA NOT available to torch -> running on CPU (slow). "
                          "Install a CUDA-enabled torch for this Python.")
        else:
            rospy.loginfo("YOLO: using GPU (cuda:0)")

        # queue_size=1 + a LARGE buff_size so rospy drains the socket buffer and
        # always processes the LATEST frame, dropping stale ones. Without a big
        # buff_size, when inference is slower than the frame rate old frames pile
        # up in the TCP buffer and the processing delay grows unboundedly (~10s).
    ...
    def image_callback(self, msg):
        try:
            ...
            results = self.model(cv_image, conf=0.6, device=self.device, verbose=False)[0] # conf: confidence threshold, verbose: terminal output
